...
function updateQuantity(index, change) {
    ...
    if (newQuantity < MIN_ORDER_QUANTITY) {
        showNotification(`Minimum order quantity is ${MIN_ORDER_QUANTITY} units per item.`);
        return;
    }
        ...
        return `
            <div class="cart-item">
                ...
                <div class="cart-item-info">
                    ...
                    <div style="font-size: 0.85rem; color: var(--text-light); margin-top: 0.25rem;">Min: 25 units</div>
                </div>
...
function proceedToCheckout() {
    ...
    const invalidItems = cart.filter(item => item.quantity < MIN_ORDER_QUANTITY);
    if (invalidItems.length > 0) {
        showNotification(`Some items don't meet the minimum order of ${MIN_ORDER_QUANTITY} units.`);
        return;
    }
...
function showThankYouMessage(itemCount, total) {
    ...
    messageDiv.innerHTML = `
        ...
        <p style="font-size: 1rem; color: var(--text-light); margin-bottom: 1.5rem;">Total: $${total} | Items: ${itemCount} units</p>
